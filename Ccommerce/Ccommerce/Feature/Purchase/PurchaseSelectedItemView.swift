...
final class PurchaseSelectedItemView: UIView {
    ...
    private var descriptionLabel: UILabel = {
        let label: UILabel = UILabel()
        label.font = CCFont.UIKit.r12
        label.textColor = CCColor.UIKit.keyColorGray2
        return label
    }()
    ...
    private func setBorder() {
        layer.borderColor = CCColor.UIKit.keyColorGray1.cgColor
        layer.borderWidth = 1
    }
    
    private func setViewModel() {
        titleLabel.text = viewModel.title
        descriptionLabel.text = viewModel.decription
        
    }
}

#Preview {
    PurchaseSelectedItemView(viewModel: PurchaseSelectedItemViewModel(title: "PlayStation5", decription: "수량 5개 / 무료배송"))
}
