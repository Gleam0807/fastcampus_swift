...
final class FavoriteViewController: UIViewController {
    ...
    private func setDataSource() -> DataSource {
        let dataSource: DataSource = UITableViewDiffableDataSource(tableView: tableView) { [weak self] tableView, indexPath, viewModel in
            ...
            case .favorite:
                return self?.favoriteCell(tableView, indexPath, viewModel: viewModel)
            case .none:
                return .init()
            }
        }
    ...
    private func favoriteCell(_ tableView: UITableView, _ indexPath: IndexPath, viewModel: AnyHashable) -> UITableViewCell {
